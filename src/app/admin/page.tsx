...
export default function AdminPage() {
  ...
  const TABS = [
    ...
    { id: "samples", label: "Sample Packs", icon: Music, count: stats.samples },
    { id: "reports", label: "Reports", icon: AlertTriangle, count: stats.reports },
    { id: "chat", label: "Chat Moderation", icon: MessageSquare, count: 0 },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            ...
            {activeTab === "samples" && (
              <button className="btn-neon btn-neon-purple text-sm !py-2.5">
          ...
          <div className="glass-card min-h-[500px] overflow-hidden">
            {isLoading ? (
              ...
              <div className="divide-y divide-border-default/50">
                ...
                {activeTab === "users" && data.map((user) => (
                  <div key={user.id} className="p-4 flex items-center justify-between hover:bg-white/5 transition-colors">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        <button className="p-2 rounded-lg hover:bg-neon-pink/10 text-text-tertiary hover:text-neon-pink transition-colors">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-text-tertiary">
                          Host: <span className="text-neon-purple">{room.host?.username}</span> • {room._count?.players} players • {room.bpm} BPM
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                ...
                {/* Samples Tab Render */}
                {activeTab === "samples" && data.map((pack) => (
                  <div key={pack.id} className="p-4 flex items-center justify-between hover:bg-white/5 transition-colors">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-neon-purple/10 flex items-center justify-center border border-neon-purple/20">
                        <FileAudio className="w-5 h-5 text-neon-purple" />
                      </div>
                      <div>
                        <div className="font-bold text-white">{pack.name}</div>
                        <div className="text-xs text-text-tertiary">{pack.genre} • {pack.files?.length || 0} files • Created {new Date(pack.createdAt).toLocaleDateString()}</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <button className="p-2 rounded-lg hover:bg-white/10 text-text-tertiary hover:text-white transition-colors">
